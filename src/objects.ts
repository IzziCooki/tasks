...
export function isCorrect(question: Question, answer: string): boolean {
    const questionTrim = question.expected.trim().toLowerCase();
    const answerTrim = answer.trim().toLowerCase();
    return questionTrim === answerTrim;
}
...
export function isValid(question: Question, answer: string): boolean {
    const answerValid = !!answer; // Converts answer to a boolean

    const questionT = question.type;

    if (questionT === 'short_answer_question') {
        return answerValid;
    } else if (questionT === 'multiple_choice_question') {
        return answerValid && question.options.includes(answer);
    }
...
export function toMarkdown(question: Question): string {
    ...
    if (question.type === 'multiple_choice_question') {
        const optionsString = question.options.map(option => `- ${option}`).join('\n');
        result += `\n${optionsString}`; 
    }
